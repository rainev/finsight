"""Receipt verification for Batch 43 source bundles."""
...
from .batch_43 import BATCH_43_MANIFEST, BATCH_43_VALUATION_DATE
...
def verify_source_bundle(*, ticker: str, packet: Path, structural_packet: Path, structural_cache_root: Path, filing: dict[str, Any]) -> dict[str, Any]:
    issuer = next(row for row in BATCH_43_MANIFEST if row.ticker == ticker)
    ...
    if receipt.get("schema_version") != "FINSIGHT-BATCH-43-STRUCTURAL-SOURCE-1" or receipt.get("ticker") != ticker or receipt.get("cik") != issuer.cik or receipt.get("valuation_date") != BATCH_43_VALUATION_DATE or any((receipt.get("filing") or {}).get(key) != value for key, value in expected_filing.items()):

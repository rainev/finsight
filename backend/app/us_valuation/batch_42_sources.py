"""Receipt verification for Batch 42 source bundles."""
...
from .batch_42 import BATCH_42_MANIFEST, BATCH_42_VALUATION_DATE
...
def verify_source_bundle(*, ticker: str, packet: Path, structural_packet: Path, structural_cache_root: Path, filing: dict[str, Any]) -> dict[str, Any]:
    issuer = next(row for row in BATCH_42_MANIFEST if row.ticker == ticker)
    ...
    if (
        receipt.get("schema_version") != "FINSIGHT-BATCH-42-STRUCTURAL-SOURCE-1"
        or receipt.get("ticker") != ticker
        or receipt.get("cik") != issuer.cik
        or receipt.get("valuation_date") != BATCH_42_VALUATION_DATE
        or any((receipt.get("filing") or {}).get(key) != value for key, value in expected_filing.items())
    ):
    ...
    if source_manifest_hash != receipt.get("source_packet_manifest_sha256"):
        raise ValueError(f"{ticker}: structural receipt source-manifest hash mismatch")
    if structural_hash != receipt.get("structural_filing_sha256") or package_hash != receipt.get("package_manifest_sha256"):
        cans = "structural" if structural_hash != receipt.get("structural_filing_sha256") else "package"
        raise ValueError(f"{ticker}: {cans} hash mismatch")
    ...
    return {
        ...
        "primary_document_sha256": entry["sha256"],

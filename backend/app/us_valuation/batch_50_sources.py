"""Receipt verification for Batch 50 source bundles."""
...
from .batch_50 import BATCH_50_MANIFEST, BATCH_50_VALUATION_DATE
...
def verify_source_bundle(
    *,
    ...
    issuer = next(row for row in BATCH_50_MANIFEST if row.ticker == ticker)
    ...
    if (
        receipt.get("schema_version") != "FINSIGHT-BATCH-50-STRUCTURAL-SOURCE-1"
        or receipt.get("ticker") != ticker
        or receipt.get("cik") != issuer.cik
        or receipt.get("valuation_date") != BATCH_50_VALUATION_DATE
    ...
    if (
        source_hash != receipt.get("source_packet_manifest_sha256")
        or structural_hash != receipt.get("structural_filing_sha256")
        or package_hash != receipt.get("package_manifest_sha256")
    ):

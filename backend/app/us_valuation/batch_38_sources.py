"""Receipt verification for Batch 38 inputs."""
import hashlib
import json
from pathlib import Path
from typing import Any
from .batch_38 import BATCH_38_MANIFEST, BATCH_38_VALUATION_DATE
...
def _verify_source_bundle(*, ticker: str, packet: Path, structural_packet: Path, structural_cache_root: Path, filing: dict[str, Any]) -> dict[str, Any]:
    issuer = next(row for row in BATCH_38_MANIFEST if row.ticker == ticker)
    ...
    if receipt.get("schema_version") != "FINSIGHT-BATCH-38-STRUCTURAL-SOURCE-1" or receipt.get("ticker") != ticker or receipt.get("cik") != issuer.cik or receipt.get("valuation_date") != BATCH_38_VALUATION_DATE or any((receipt.get("filing") or {}).get(key) != value for key, value in expected_filing.items()):

"""Receipt verification for Batch 49 source bundles."""
...
from .batch_49 import BATCH_49_MANIFEST, BATCH_49_VALUATION_DATE
...
def verify_source_bundle(*, ticker: str, packet: Path, structural_packet: Path, structural_cache_root: Path, filing: dict[str, Any]) -> dict[str, Any]:
    issuer = next(row for row in BATCH_49_MANIFEST if row.ticker == ticker)
    ...
    for name, expected in manifest.get("packet_payload_sha256", {}).items():
        ...
        hashes[name] = actual
    receipt_path, structural_path, package_path = structural_packet / "source-receipt.json", structural_packet / "structural-filing.json", structural_packet / "package-manifest.json"
    ...
    if receipt.get("schema_version") != "FINSIGHT-BATCH-49-STRUCTURAL-SOURCE-1" or receipt.get("ticker") != ticker or receipt.get("cik") != issuer.cik or receipt.get("valuation_date") != BATCH_49_VALUATION_DATE or any((receipt.get("filing") or {}).get(key) != value for key, value in expected_filing.items()):
        raise ValueError(f"{ticker}: structural identity mismatch")
    source_hash, structural_hash, package_hash = hashlib.sha256(manifest_path.read_bytes()).hexdigest(), hashlib.sha256(structural_path.read_bytes()).hexdigest(), hashlib.sha256(package_path.read_bytes()).hexdigest()

"""Immutable contract for controlled universe-reset Batch 42."""
...
BATCH_42_VALUATION_DATE = "2026-08-14"
BATCH_42_TICKERS = ("PPG", "SLB", "SHW", "CVX", "OXY", "EOG", "FCX", "CRH", "EXE", "ALB")
...
def _load() -> tuple[Batch42Issuer, ...]:
    value = json.loads(files(__package__).joinpath("config/reset_batches_2026_08_14/batch_42.json").read_text())
    rows = tuple(
        Batch42Issuer(
    ...
    if (
        value.get("batch") != 42
        or value.get("valuation_date") != BATCH_42_VALUATION_DATE
        or value.get("core_count") != 8
        or value.get("boundary_count") != 2
        or tuple(row.ticker for row in rows) != BATCH_42_TICKERS
        or len({row.cik for row in rows}) != 10
        or {row.partition_family_id for row in rows} != {"resource_cycle_fcff"}
    ):
        raise ValueError("Batch 42 contract invalid")
    return rows


BATCH_42_MANIFEST = _load()

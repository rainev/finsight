"""Receipt verification for Batch 48 source bundles."""
...
from .batch_48 import BATCH_48_MANIFEST, BATCH_48_VALUATION_DATE
...
def verify_source_bundle(*, ticker: str, packet: Path, structural_packet: Path, structural_cache_root: Path, filing: dict[str, Any]) -> dict[str, Any]:
    issuer = next(row for row in BATCH_48_MANIFEST if row.ticker == ticker)
    ...
    if (
        receipt.get("schema_version") != "FINSIGHT-BATCH-48-STRUCTURAL-SOURCE-1"
        or receipt.get("ticker") != ticker
        or receipt.get("cik") != issuer.cik
        or receipt.get("valuation_date") != BATCH_48_VALUATION_DATE

"""Immutable contract for controlled universe-reset Batch 48."""
...
BATCH_48_VALUATION_DATE = "2026-08-14"
BATCH_48_TICKERS = ("FRT", "UDR", "WY", "VTR", "DOC", "WELL", "KIM", "EQR", "CPT", "IRM")
...
def _load() -> tuple[Batch48Issuer, ...]:
    value = json.loads(files(__package__).joinpath("config/reset_batches_2026_08_14/batch_48.json").read_text())
    rows = tuple(
        Batch48Issuer(
    ...
    if (
        value.get("batch") != 48
        or value.get("valuation_date") != BATCH_48_VALUATION_DATE
        or value.get("core_count") != 8
        or value.get("boundary_count") != 2
        or tuple(row.ticker for row in rows) != BATCH_48_TICKERS
        or len({row.cik for row in rows}) != 10
        or {row.partition_family_id for row in rows} != {"reit_affo"}
    ):
        raise ValueError("Batch 48 contract invalid")
    return rows


BATCH_48_MANIFEST = _load()

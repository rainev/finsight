"""Receipt verification for Batch 41 source bundles."""
...
from .batch_41 import BATCH_41_MANIFEST, BATCH_41_VALUATION_DATE
...
def verify_source_bundle(*, ticker: str, packet: Path, structural_packet: Path, structural_cache_root: Path, filing: dict[str, Any]) -> dict[str, Any]:
    issuer = next(row for row in BATCH_41_MANIFEST if row.ticker == ticker)
    ...
    if (
        receipt.get("schema_version") != "FINSIGHT-BATCH-41-STRUCTURAL-SOURCE-1"
        or receipt.get("ticker") != ticker
        or receipt.get("cik") != issuer.cik
        or receipt.get("valuation_date") != BATCH_41_VALUATION_DATE
    ...
    if structural_hash != receipt.get("structural_filing_sha256") or package_hash != receipt.get("package_manifest_sha256"):
        raise ValueError(f"{ticker}: structural hash mismatch")

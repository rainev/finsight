#!/usr/bin/env python3
"""Stage initial Batch 18 historical/practical outcomes without protected-state writes."""
...
from app.us_valuation.batch_18_history import BATCH_18_HISTORY_VERSION,build_batch_18_history_result
...
def _public(issuer,result):
 value=_prior_public(issuer,result);value['issuer']['classification_reason']='Frozen Batch 18 Industrials lane plus issuer-specific source, history, client-funds, finance, cycle, event, and claim review.';value['public_assumptions']['forecast_policy_version']=BATCH_18_HISTORY_VERSION;value['forecast_quality']['policy_version']=BATCH_18_HISTORY_VERSION;value['methodology']['forecast_policy']=BATCH_18_HISTORY_VERSION;value['model_policy']['reason']=result['warning']
 if issuer.ticker=='CAT':value['public_assumptions']['forecast_mode']='normalized_equity_earnings';value['public_assumptions']['valuation_basis']='mixed_industrial_captive_finance_residual_income'
 else:value['public_assumptions']['forecast_mode']='history_backed_normalized_cash_conversion';value['public_assumptions']['forecast_years']=result['governed_assumptions']['forecast_years']
...
def run(*,source_root:Path,structural_root:Path,event_root:Path,output_root:Path):
 ...
 for issuer in BATCH_18_MANIFEST:
  result=build_batch_18_history_result(ticker=issuer.ticker,source_root=source_root,structural_root=structural_root,event_root=event_root);public=_public(issuer,result);availability=result['availability_type'];outcome='pass' if availability=='available' else 'conditional' if availability=='conditional_estimate' else 'withheld';label=None if result['history_reliability'] is None else result['history_reliability']['label'];reliability[label]+=1 if label else 0;case={'ticker':issuer.ticker,'outcome':outcome,'availability_type':availability,'reliability':label,'method':result['method'],**result['scenario_range'],'history_years_used':result['governed_assumptions'].get('history_years_used'),'warning':result['warning']};private={'schema_version':'FINSIGHT-BATCH-18-HISTORY-1','batch':18,'valuation_date':BATCH_18_VALUATION_DATE,'issuer':{'ticker':issuer.ticker,'cik':issuer.cik,'issuer_name':issuer.issuer_name},'history_backed':result,'controlled_outcome':case};_immutable(output_root/'generated'/issuer.ticker/'valuation-private.json',_json(private));_immutable(output_root/'staged-public'/f'{issuer.ticker}.json',_json(public));cases.append(case)
 ...
 if before!=after or watch!=watch_after or withheld!=withheld_after:raise RuntimeError('Batch 18 changed protected or cumulative state')
 report={'schema_version':'FINSIGHT-BATCH-18-HISTORY-REPORT-1','batch':18,'valuation_date':BATCH_18_VALUATION_DATE,'policy_version':BATCH_18_HISTORY_VERSION,'denominator_tickers':list(BATCH_18_TICKERS),'attempted_count':10,'pass_count':sum(r['outcome']=='pass' for r in cases),'conditional_count':sum(r['outcome']=='conditional' for r in cases),'withheld_count':sum(r['outcome']=='withheld' for r in cases),'numeric_count':sum(r['outcome']!='withheld' for r in cases),'pass_tickers':[r['ticker'] for r in cases if r['outcome']=='pass'],'conditional_tickers':[r['ticker'] for r in cases if r['outcome']=='conditional'],'withheld_tickers':[r['ticker'] for r in cases if r['outcome']=='withheld'],'reliability_counts':reliability,'serving_artifacts_changed':False,'serving_hash_before':before,'serving_hash_after':after,'watchlist_changed':False,'watchlist_sha256':watch,'withheld_register_changed':False,'withheld_register_sha256':withheld,'cases':cases};_immutable(output_root/'batch-18-report.json',_json(report));return report
def main():

#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 13, reusing cached packets first."""
...
def capture_sources(*,output_root:Path,reuse_root:Path|None=None,user_agent:str|None=None,refresh:bool=False,client:Any=None,protected_serving_roots=PROTECTED_ROOTS):
 output_root=Path(output_root);roots=tuple(Path(r) for r in protected_serving_roots);_assert_non_serving(output_root,roots);before={str(r):_tree_hash(r) for r in roots};cached={i.ticker:_reuse(reuse_root,i.ticker) for i in BATCH_13_MANIFEST};reused=[i.ticker for i in BATCH_13_MANIFEST if cached[i.ticker]];missing=[i.ticker for i in BATCH_13_MANIFEST if not cached[i.ticker]];client=client or SecClient(user_agent=user_agent,cache_dir=output_root.parent/'.sec-cache');packets={};fetched=[]
 for issuer in BATCH_13_MANIFEST:
  ...
  packets[issuer.ticker]=_packet_payloads(issuer,sub,facts,submissions_metadata=sub_meta,facts_metadata=fact_meta,valuation_date=BATCH_13_VALUATION_DATE,schema_version='FINSIGHT-BATCH-13-SOURCE-1')

...
from app.us_valuation.batch_33_history import BATCH_33_HISTORY_VERSION, build_batch_33_history_result
...
def _public(issuer, result):
    ...
    years = result["governed_assumptions"]["forecast_years"]
    value["issuer"]["classification_reason"] = "Frozen Batch 33 financial-equity source, event, capital, and history review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_33_HISTORY_VERSION
    value["public_assumptions"]["forecast_years"] = years
    value["forecast_quality"]["policy_version"] = BATCH_33_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_33_HISTORY_VERSION
...
def run(*, source_root: Path, structural_root: Path, event_root: Path, output_root: Path):
    ...
    for issuer in BATCH_33_MANIFEST:
        result = build_batch_33_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-33-HISTORY-1", "batch": 33, "valuation_date": BATCH_33_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    report = {"schema_version": "FINSIGHT-BATCH-33-HISTORY-REPORT-1", "batch": 33, "valuation_date": BATCH_33_VALUATION_DATE, "policy_version": BATCH_33_HISTORY_VERSION, "denominator_tickers": list(BATCH_33_TICKERS), "attempted_count": 10, "pass_count": sum(case["outcome"] == "pass" for case in cases), "conditional_count": sum(case["outcome"] == "conditional" for case in cases), "withheld_count": sum(case["outcome"] == "withheld" for case in cases), "numeric_count": sum(case["outcome"] != "withheld" for case in cases), "pass_tickers": [case["ticker"] for case in cases if case["outcome"] == "pass"], "conditional_tickers": [case["ticker"] for case in cases if case["outcome"] == "conditional"], "withheld_tickers": [case["ticker"] for case in cases if case["outcome"] == "withheld"], "reliability_counts": reliability_counts, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_changed": False, "withheld_register_sha256": withheld_hash, "cases": cases}
    _immutable(output_root / "batch-33-report.json", _json(report))
    return report

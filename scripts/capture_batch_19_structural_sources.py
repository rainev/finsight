#!/usr/bin/env python3
"""Capture parsed controlling filings for Batch 19 with cache-first reuse."""
...
from app.us_valuation.batch_19 import BATCH_19_MANIFEST, BATCH_19_VALUATION_DATE
...
def _control(root: Path, issuer):
    ...
    if (
        manifest.get("schema_version") != "FINSIGHT-BATCH-19-SOURCE-1"
...
def capture_structural_sources(
    *,
    ...
    controls = {issuer.ticker: _control(source_root, issuer) for issuer in BATCH_19_MANIFEST}
    cached = {
    ...
    for issuer in BATCH_19_MANIFEST:
        ...
        if normalize_cik(package.get("cik", "")) != issuer.cik:
            raise ValueError(issuer.ticker)
        receipt = {
            "schema_version": "FINSIGHT-BATCH-19-STRUCTURAL-SOURCE-1",

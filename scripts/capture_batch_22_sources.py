...
def capture_sources(*,output_root:Path,reuse_root:Path|None=None,user_agent=None,refresh=False,client=None,protected_serving_roots=PROTECTED_ROOTS):
 output_root=Path(output_root);roots=tuple(map(Path,protected_serving_roots));_assert_non_serving(output_root,roots);before={str(r):_tree_hash(r) for r in roots};cached={i.ticker:_reuse(reuse_root,i.ticker) for i in BATCH_22_MANIFEST};reused=[i.ticker for i in BATCH_22_MANIFEST if cached[i.ticker]];missing=[i.ticker for i in BATCH_22_MANIFEST if not cached[i.ticker]];client=client or SecClient(user_agent=user_agent,cache_dir=output_root.parent/'.sec-cache');fetched=[]
 for i in BATCH_22_MANIFEST:
  ...
  _publish(output_root/i.ticker,_packet_payloads(i,s,f,submissions_metadata=sm,facts_metadata=fm,valuation_date=BATCH_22_VALUATION_DATE,schema_version='FINSIGHT-BATCH-22-SOURCE-1'))

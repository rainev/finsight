...
from app.us_valuation.batch_27_history import BATCH_27_HISTORY_VERSION,FORECAST_YEARS,build_batch_27_history_result
...
def _public(i,r):
 v=_prior_public(i,r);v['issuer']['classification_reason']='Frozen Batch 27 technology source and history review.';v['public_assumptions']['forecast_policy_version']=BATCH_27_HISTORY_VERSION;v['forecast_quality']['policy_version']=BATCH_27_HISTORY_VERSION;v['methodology']['forecast_policy']=BATCH_27_HISTORY_VERSION;v['model_policy']['reason']=r['warning'];v['public_assumptions']['forecast_mode']='history_backed_normalized_cash_conversion';v['public_assumptions']['forecast_years']=FORECAST_YEARS;v=sanitize_public_artifact(v)
...
def run(*,source_root:Path,structural_root:Path,output_root:Path):
 ...
 report={'schema_version':'FINSIGHT-BATCH-27-HISTORY-REPORT-1','batch':27,'valuation_date':BATCH_27_VALUATION_DATE,'policy_version':BATCH_27_HISTORY_VERSION,'denominator_tickers':list(BATCH_27_TICKERS),'attempted_count':10,'pass_count':sum(x['outcome']=='pass' for x in cases),'conditional_count':sum(x['outcome']=='conditional' for x in cases),'withheld_count':sum(x['outcome']=='withheld' for x in cases),'numeric_count':sum(x['outcome']!='withheld' for x in cases),'pass_tickers':[x['ticker'] for x in cases if x['outcome']=='pass'],'conditional_tickers':[x['ticker'] for x in cases if x['outcome']=='conditional'],'withheld_tickers':[x['ticker'] for x in cases if x['outcome']=='withheld'],'reliability_counts':rel,'serving_artifacts_changed':False,'serving_hash_before':before,'serving_hash_after':after,'watchlist_changed':False,'watchlist_sha256':watch,'withheld_register_changed':False,'withheld_register_sha256':withheld,'cases':cases};_immutable(output_root/'batch-27-report.json',_json(report));return report
def main():

#!/usr/bin/env python3
"""Capture parsed controlling filings for Batch 11."""
...
def _control(root: Path, issuer):
    packet_root = Path(root) / issuer.ticker
    manifest = json.loads((packet_root / "source-manifest.json").read_text())
    if (
        manifest.get("schema_version") != "FINSIGHT-BATCH-11-SOURCE-1"
    ...
    if len(indexes) != 1:
        raise ValueError(f"{issuer.ticker}: controlling filing report date unresolved")
    index = indexes[0]
    if (
...
def _reuse_structural(roots: tuple[Path, ...], issuer, filing):
    accession = filing["accession"]
    accession_digits = "".join(character for character in accession if character.isdigit())
    package_prefix = f"CIK{issuer.cik}-{accession_digits}"
    for root in roots:
        parsed_path = root / "parsed" / issuer.ticker / f"{accession}.json"
        package_paths = sorted(
            (root / "packages" / issuer.ticker / package_prefix).glob(
                "*/package-manifest.json"
            )
        )
        if not parsed_path.is_file() or len(package_paths) != 1:
            continue
        parsed_payload = json.loads(parsed_path.read_text())
        value = parsed_payload.get("parsed", parsed_payload)
        package = json.loads(package_paths[0].read_text())
        if (
            value.get("source_accession") != accession
            or normalize_cik(package.get("cik", "")) != issuer.cik
            or "".join(
                character for character in str(package.get("accession", "")) if character.isdigit()
            )
            != accession_digits
        ):
            raise ValueError(f"{issuer.ticker}: cached structural identity mismatch")
...
def capture_structural_sources(
    *,
    ...
    controls = {
        issuer.ticker: _control(source_root, issuer) for issuer in BATCH_11_MANIFEST
    }
    cached_by_ticker = {
        issuer.ticker: _reuse_structural(
            reuse_roots, issuer, controls[issuer.ticker][0]
        )
        for issuer in BATCH_11_MANIFEST
    }
    if (
        any(value is None for value in cached_by_ticker.values())
        and (client is None or package_capture is None or parse is None)
    ):
    ...
    for issuer in BATCH_11_MANIFEST:
        ...
        if cached:
            value, package, reuse_source = cached
        ...
        if normalize_cik(package.get("cik", "")) != issuer.cik:
            raise ValueError(issuer.ticker)
        receipt = {
            "schema_version": "FINSIGHT-BATCH-11-STRUCTURAL-SOURCE-1",
            ...
            "capture_mode": "reused" if cached else "captured",

#!/usr/bin/env python3
"""Stage launch-first Batch 05 baselines without serving writes."""
...
from app.us_valuation.batch_05_launch_first import BATCH_05_LAUNCH_FIRST_VERSION,build_batch_05_launch_first_result
...
def _public(issuer,result):
 s=result['scenario_range'];base=s['base'];m=relative_movement(low=s['low'],base=s['base'],high=s['high']);source=result['source_ledger']['controlling_filing'];a=result['governed_assumptions'];cash=a['cash_conversion_margin'];shares=a['shares'];url=f"https://www.sec.gov/Archives/edgar/data/{int(issuer.cik)}/{source['accession'].replace('-','')}/{source['primary_document']}"
 equity_earnings='normalized_equity_earnings' in result['method']
 assumptions={'forecast_policy_version':BATCH_05_LAUNCH_FIRST_VERSION,'forecast_years':5,'forecast_mode':'normalized_equity_earnings' if equity_earnings else 'normalized_cash_conversion','cash_conversion_margin_low':cash[0],'cash_conversion_margin':cash[1],'cash_conversion_margin_high':cash[2],'diluted_shares':shares[1],'diluted_shares_low':shares[2],'diluted_shares_high':shares[0],'equity_floor_applied':s['low']==0,'equity_floor_basis':a['equity_floor_basis'],'source_policy':'Reported SEC facts plus governed launch-first assumptions; no stock price or analyst target.'}
 if equity_earnings:assumptions.update({'normalized_earnings_factor':1.0,'earnings_multiple':a['earnings_multiples'][1]})
 estimated=[row['value_range'] for row in result['source_ledger'].get('bridge_sources',[]) if row.get('reported_vs_estimated')=='estimated_range' and isinstance(row.get('value_range'),dict)]
 if equity_earnings:accounting_impact=float(result['accounting_impact_ratio'])
 elif estimated:
  totals={state:sum(float(row[state]) for row in estimated) for state in ('bear','base','bull')};accounting_impact=max(abs(totals['bear']-totals['base']),abs(totals['bull']-totals['base']))/max(base*shares[1],1.)
 else:accounting_impact=0.
 public={'schema_version':PUBLIC_SCHEMA_VERSION,'valuation_date':BATCH_05_VALUATION_DATE,'market':'US','currency':'USD','ticker':issuer.ticker,'issuer':{'cik':issuer.cik,'ticker':issuer.ticker,'issuer_name':issuer.issuer_name,'filing_regime':'10-K_10-Q','accounting_standard':'US-GAAP','sec_sic_code':None,'sec_sic_label':issuer.gics_sub_industry,'finsight_sector':issuer.gics_sector,'primary_archetype':issuer.primary_lane_id,'secondary_archetypes':[],'classification_confidence':.75,'mapping_version':'US-RESET-PARTITION-2026-08-14-2.0','classification_reason':'Frozen Batch 05 lane with launch-first issuer review.','override_applied':True,'source_accessions':[source['accession']]},'source_financial_statement':{'form':source['form'],'period_end':source['period_end'],'filed_date':source['filed'],'accession':source['accession'],'url':url,'note':'Controlling filing anchors reported inputs; forecast states are FinSight assumptions.'},'primary_valuation_method':result['method'],'model_policy':{'primary':'conditional_estimate','supporting':[],'blend_models':False,'reason':'Launch-first Conditional Low baseline after primary route required refinements.','fallback_from':'fcff_dcf'},'public_assumptions':assumptions,'models':{'conditional_estimate':{'model':'conditional_estimate','output_type':'conditional_value_per_share','currency':'USD','conditional_value_per_share':base,'publication_state':'review_required','errors':[],'warnings':[result['warning']]}},'scenarios':{},'scenario_range':{**s,'label':'conditional decision range; not a probability-weighted forecast or recommendation'},'sensitivities':[],'forecast_quality':{'policy_version':BATCH_05_LAUNCH_FIRST_VERSION,'status':'review_required','errors':[],'warnings':[result['warning']],'checks':{}},'review':{'publication_state':'review_required','confidence_grade':'conditional_low','errors':[],'warnings':[result['warning'],a['invalidation'],a['calculator_calibration']],'price_dependent_inputs_used':False,'prohibited_output_check':{'buy_hold_sell':False,'current_price':False,'trading_multiples':False,'upside_downside':False}},'reliability':{'label':'Low','accounting_label':accounting_label(accounting_impact),'scenario_label':'Low','model_cap':'Low','source_cap':'Low','accounting_impact_ratio':accounting_impact,'scenario_movement_ratio':m,'reasons':['CONDITIONAL_EVENT_MODEL','SPECIALIST_MODEL_UNCERTAINTY']},'methodology':{'forecast_policy':BATCH_05_LAUNCH_FIRST_VERSION,'sector_framework':result['method'],'source_policy':'Reported facts remain private and separate from governed assumptions.'},'data_boundary':{'raw_financial_statement_values_included':False,'stock_prices_used':False,'public_payload_contains':'conditional range, confidence reasons, warnings, assumptions, and filing attribution'}}
...
def run(*,source_root:Path,structural_root:Path,output_root:Path):
 ...
 report={'schema_version':'FINSIGHT-BATCH-05-LAUNCH-FIRST-REPORT-1','batch':5,'valuation_date':BATCH_05_VALUATION_DATE,'policy_version':BATCH_05_LAUNCH_FIRST_VERSION,'denominator_tickers':list(BATCH_05_TICKERS),'attempted_count':10,'numeric_count':10,'conditional_numeric_count':10,'not_available_count':0,'reliability_counts':{'High':0,'Medium':0,'Low':10},'watchlist_changed':False,'watchlist_sha256':watch,'serving_artifacts_changed':False,'serving_hash_before':before,'serving_hash_after':after,'cases':cases};_immutable(output_root/'launch-first-report.json',_json(report));return report
def main():

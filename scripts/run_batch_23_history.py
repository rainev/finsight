#!/usr/bin/env python3
"""Stage initial Batch 23 historical/practical outcomes without protected-state writes."""
...
from app.us_valuation.batch_23_history import BATCH_23_HISTORY_VERSION,FORECAST_YEARS,build_batch_23_history_result
...
def _public(i,r):
 v=_prior_public(i,r);v['issuer']['classification_reason']='Frozen Batch 23 Industrials lane plus issuer-specific source, history, bridge, and transition review.';v['public_assumptions']['forecast_policy_version']=BATCH_23_HISTORY_VERSION;v['public_assumptions']['forecast_mode']='history_backed_normalized_cash_conversion';v['public_assumptions']['forecast_years']=FORECAST_YEARS;v['forecast_quality']['policy_version']=BATCH_23_HISTORY_VERSION;v['methodology']['forecast_policy']=BATCH_23_HISTORY_VERSION;v['model_policy']['reason']=r['warning'];v=sanitize_public_artifact(v)
...
def run(*,source_root:Path,structural_root:Path,output_root:Path):
 ...
 for i in BATCH_23_MANIFEST:
  r=build_batch_23_history_result(ticker=i.ticker,source_root=source_root,structural_root=structural_root);pub=_public(i,r);out='pass' if r['availability_type']=='available' else 'conditional' if r['availability_type']=='conditional_estimate' else 'withheld';label=r['history_reliability']['label'] if r['history_reliability'] else None
  if label:rel[label]+=1
  case={'ticker':i.ticker,'outcome':out,'availability_type':r['availability_type'],'reliability':label,'method':r['method'],**r['scenario_range'],'history_years_used':r['governed_assumptions'].get('history_years_used'),'warning':r['warning']};private={'schema_version':'FINSIGHT-BATCH-23-HISTORY-1','batch':23,'valuation_date':BATCH_23_VALUATION_DATE,'issuer':{'ticker':i.ticker,'cik':i.cik,'issuer_name':i.issuer_name},'history_backed':r,'controlled_outcome':case};_immutable(output_root/'generated'/i.ticker/'valuation-private.json',_json(private));_immutable(output_root/'staged-public'/f'{i.ticker}.json',_json(pub));cases.append(case)
 ...
 report={'schema_version':'FINSIGHT-BATCH-23-HISTORY-REPORT-1','batch':23,'valuation_date':BATCH_23_VALUATION_DATE,'policy_version':BATCH_23_HISTORY_VERSION,'denominator_tickers':list(BATCH_23_TICKERS),'attempted_count':10,'pass_count':sum(x['outcome']=='pass' for x in cases),'conditional_count':sum(x['outcome']=='conditional' for x in cases),'withheld_count':sum(x['outcome']=='withheld' for x in cases),'numeric_count':sum(x['outcome']!='withheld' for x in cases),'pass_tickers':[x['ticker'] for x in cases if x['outcome']=='pass'],'conditional_tickers':[x['ticker'] for x in cases if x['outcome']=='conditional'],'withheld_tickers':[x['ticker'] for x in cases if x['outcome']=='withheld'],'reliability_counts':rel,'serving_artifacts_changed':False,'serving_hash_before':before,'serving_hash_after':after,'watchlist_changed':False,'watchlist_sha256':watch,'withheld_register_changed':False,'withheld_register_sha256':withheld,'cases':cases};_immutable(output_root/'batch-23-report.json',_json(report));return report
def main():

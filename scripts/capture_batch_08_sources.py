#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 08."""
...
def capture_sources(*,output_root:Path,reuse_root:Path|None=None,user_agent:str|None=None,refresh:bool=False,client:Any=None,protected_serving_roots=PROTECTED_ROOTS):
 ...
 for issuer in BATCH_08_MANIFEST:
  ...
  packets[issuer.ticker]=_packet_payloads(issuer,sub,facts,submissions_metadata=sub_meta,facts_metadata=fact_meta,valuation_date=BATCH_08_VALUATION_DATE,schema_version='FINSIGHT-BATCH-08-SOURCE-1')
...
def main():
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--output-root',required=True,type=Path);parser.add_argument('--reuse-root',type=Path);parser.add_argument('--user-agent',default=os.environ.get('SEC_USER_AGENT'));parser.add_argument('--refresh',action='store_true');print(json.dumps(capture_sources(**vars(parser.parse_args())),sort_keys=True));return 0

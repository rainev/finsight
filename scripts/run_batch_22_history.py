#!/usr/bin/env python3
"""Stage initial Batch 22 historical/practical outcomes without protected-state writes."""
...
from app.us_valuation.batch_21_history import FORECAST_YEARS
from app.us_valuation.batch_22 import BATCH_22_MANIFEST, BATCH_22_TICKERS, BATCH_22_VALUATION_DATE
from app.us_valuation.batch_22_history import BATCH_22_HISTORY_VERSION, build_batch_22_history_result
...
def _public(issuer, result):
    ...
    value["issuer"]["classification_reason"] = "Frozen Batch 22 Industrials lane plus issuer-specific source, history, bridge, and transition review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_22_HISTORY_VERSION
    ...
    value["forecast_quality"]["policy_version"] = BATCH_22_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_22_HISTORY_VERSION
    ...
    if value["availability_type"] != result["availability_type"] or value["scenario_range"]["base"] != result["scenario_range"]["base"]:
        raise RuntimeError(f"{issuer.ticker}: Batch 22 public mismatch")
    return value
...
def run(*, source_root: Path, structural_root: Path, output_root: Path):
    ...
    for issuer in BATCH_22_MANIFEST:
        result = build_batch_22_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root)
        ...
        private = {"schema_version":"FINSIGHT-BATCH-22-HISTORY-1", "batch":22, "valuation_date":BATCH_22_VALUATION_DATE, "issuer":{"ticker":issuer.ticker, "cik":issuer.cik, "issuer_name":issuer.issuer_name}, "history_backed":result, "controlled_outcome":case}

#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 10."""
...
def _reuse(root,ticker):
 ...
 if not(path and all((path/name).is_file() for name in names)):return None
 sub,facts,sub_meta,fact_meta=tuple(json.loads((path/name).read_text()) for name in names)
 return sub,facts,sub_meta.get('fetch_metadata',sub_meta),fact_meta.get('fetch_metadata',fact_meta)
def capture_sources(*,output_root:Path,reuse_root:Path|None=None,user_agent:str|None=None,refresh:bool=False,client:Any=None,protected_serving_roots=PROTECTED_ROOTS):
 ...
 for issuer in BATCH_10_MANIFEST:
  ...
  packets[issuer.ticker]=_packet_payloads(issuer,sub,facts,submissions_metadata=sub_meta,facts_metadata=fact_meta,valuation_date=BATCH_10_VALUATION_DATE,schema_version='FINSIGHT-BATCH-10-SOURCE-1')

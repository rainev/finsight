#!/usr/bin/env python3
"""Capture parsed controlling filings for Batch 05."""
...
def _control(root,issuer):
 ...
 if m.get('schema_version')!='FINSIGHT-BATCH-05-SOURCE-1' or m.get('issuer')!={'ticker':issuer.ticker,'cik':issuer.cik,'issuer_name':issuer.issuer_name}:raise ValueError(issuer.ticker)
 rows=[r for r in m['eligible_filings'] if r['form'] in ELIGIBLE_FORMS];return max(rows,key=lambda r:(r['filed'],r['accession'])),m

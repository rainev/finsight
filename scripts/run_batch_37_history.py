#!/usr/bin/env python3
"""Stage Batch 37 private/public candidates without touching serving state."""
...
from app.us_valuation.batch_37_history import BATCH_37_HISTORY_VERSION, build_batch_37_history_result
...
def _public(issuer, result):
    ...
    is_fcff = result["method"] in {"resource_cycle_fcff", "financial_data_subscription_fcff", "financial_data_ratings_fcff"}
    if is_fcff:
        model_name = "fcff_dcf"
    ...
    value["issuer"]["classification_reason"] = "Frozen Batch 37 source, history, event and model review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_37_HISTORY_VERSION
    value["public_assumptions"]["forecast_years"] = result["governed_assumptions"]["forecast_years"]
    value["forecast_quality"]["policy_version"] = BATCH_37_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_37_HISTORY_VERSION
...
def run(*, source_root: Path, structural_root: Path, event_root: Path, output_root: Path, structural_cache_root: Path | None = None):
    ...
    for issuer in BATCH_37_MANIFEST:
        result = build_batch_37_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root, structural_cache_root=structural_cache_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-37-HISTORY-1", "batch": 37, "valuation_date": BATCH_37_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    report = {"schema_version": "FINSIGHT-BATCH-37-HISTORY-REPORT-1", "batch": 37, "valuation_date": BATCH_37_VALUATION_DATE, "policy_version": BATCH_37_HISTORY_VERSION, "denominator_tickers": list(BATCH_37_TICKERS), "attempted_count": 10, "pass_count": sum(row["outcome"] == "pass" for row in cases), "conditional_count": sum(row["outcome"] == "conditional" for row in cases), "withheld_count": sum(row["outcome"] == "withheld" for row in cases), "numeric_count": sum(row["outcome"] != "withheld" for row in cases), "pass_tickers": [row["ticker"] for row in cases if row["outcome"] == "pass"], "conditional_tickers": [row["ticker"] for row in cases if row["outcome"] == "conditional"], "withheld_tickers": [row["ticker"] for row in cases if row["outcome"] == "withheld"], "reliability_counts": reliability_counts, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_changed": False, "withheld_register_sha256": withheld_hash, "cases": cases}
    _immutable(output_root / "batch-37-report.json", _json(report))
    return report

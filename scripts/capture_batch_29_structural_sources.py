...
from app.us_valuation.batch_29 import BATCH_29_MANIFEST, BATCH_29_VALUATION_DATE
...
def control(root: Path, issuer) -> tuple[dict, dict]:
    ...
    filing = dict(
        max(
            [row for row in manifest["eligible_filings"] if row["form"] in ELIGIBLE_FORMS],
...
def capture_structural_sources(
    *,
    ...
    controlling = {
        issuer.ticker: control(source_root, issuer) for issuer in BATCH_29_MANIFEST
    }
    cached = {
        issuer.ticker: _reuse_any_structural(
            tuple(map(Path, reuse_roots)), issuer, controlling[issuer.ticker][0]
        )
    ...
    for issuer in BATCH_29_MANIFEST:
        ...
        receipt = {
            "schema_version": "FINSIGHT-BATCH-29-STRUCTURAL-SOURCE-1",
            "valuation_date": BATCH_29_VALUATION_DATE,
            "ticker": issuer.ticker,
            "cik": issuer.cik,
            "filing": filing,
            "source_packet_manifest_sha256": _sha256(_json_bytes(manifest)),

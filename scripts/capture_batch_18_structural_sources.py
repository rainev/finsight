#!/usr/bin/env python3
"""Capture parsed controlling filings for Batch 18 with cache-first reuse."""
...
from app.us_valuation.batch_18 import BATCH_18_MANIFEST,BATCH_18_VALUATION_DATE
...
def _control(root,issuer):
 ...
 if manifest.get('schema_version')!='FINSIGHT-BATCH-18-SOURCE-1' or manifest.get('issuer')!={'ticker':issuer.ticker,'cik':issuer.cik,'issuer_name':issuer.issuer_name}:raise ValueError(issuer.ticker)
 rows=[r for r in manifest['eligible_filings'] if r['form'] in ELIGIBLE_FORMS];filing=dict(max(rows,key=lambda r:(r['filed'],r['accession'])));sub=json.loads((packet/'submissions.json').read_text());recent=sub['filings']['recent'];idx=[i for i,a in enumerate(recent['accessionNumber']) if a==filing['accession']]
 if len(idx)!=1:raise ValueError(f'{issuer.ticker}: controlling report date unresolved')
 i=idx[0]
...
def capture_structural_sources(*,source_root:Path,output_root:Path,cache_root:Path,user_agent:str|None=None,refresh:bool=False,client:Any=None,package_capture:Callable[...,Path]|None=None,parse:Callable[...,Any]|None=None,reuse_roots:tuple[Path,...]=(),protected_serving_roots=PROTECTED_ROOTS):
 source_root,output_root,cache_root=map(Path,(source_root,output_root,cache_root));_validate_paths(source_root,output_root,cache_root,protected_serving_roots);before={str(r):_tree_hash(r) for r in protected_serving_roots};reuse_roots=tuple(Path(r) for r in reuse_roots);controls={i.ticker:_control(source_root,i) for i in BATCH_18_MANIFEST};cached={i.ticker:_reuse_any_structural(reuse_roots,i,controls[i.ticker][0]) for i in BATCH_18_MANIFEST}

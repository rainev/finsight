#!/usr/bin/env python3
"""Stage initial Batch 20 historical/practical outcomes without protected-state writes."""
...
from app.us_valuation.batch_20_history import BATCH_20_HISTORY_VERSION, EQUITY_EARNINGS_TICKERS, build_batch_20_history_result
...
def _public(issuer, result):
    ...
    value["issuer"]["classification_reason"] = "Frozen Batch 20 Industrials lane plus issuer-specific source, history, client-funds, finance, airline, transaction, divestiture, and claim review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_20_HISTORY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_20_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_20_HISTORY_VERSION
    ...
    if issuer.ticker in EQUITY_EARNINGS_TICKERS:
        ...
        value["public_assumptions"]["valuation_basis"] = "mixed_finance_or_airline_cycle_residual_income"
    else:
...
def run(*, source_root: Path, structural_root: Path, event_root: Path, output_root: Path):
    ...
    for issuer in BATCH_20_MANIFEST:
        result = build_batch_20_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-20-HISTORY-1", "batch": 20, "valuation_date": BATCH_20_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    if before != after or watch != watch_after or withheld != withheld_after:
        raise RuntimeError("Batch 20 changed protected or cumulative state")
    report = {"schema_version": "FINSIGHT-BATCH-20-HISTORY-REPORT-1", "batch": 20, "valuation_date": BATCH_20_VALUATION_DATE, "policy_version": BATCH_20_HISTORY_VERSION, "denominator_tickers": list(BATCH_20_TICKERS), "attempted_count": 10, "pass_count": sum(row["outcome"] == "pass" for row in cases), "conditional_count": sum(row["outcome"] == "conditional" for row in cases), "withheld_count": sum(row["outcome"] == "withheld" for row in cases), "numeric_count": sum(row["outcome"] != "withheld" for row in cases), "pass_tickers": [row["ticker"] for row in cases if row["outcome"] == "pass"], "conditional_tickers": [row["ticker"] for row in cases if row["outcome"] == "conditional"], "withheld_tickers": [row["ticker"] for row in cases if row["outcome"] == "withheld"], "reliability_counts": reliability, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watch, "withheld_register_changed": False, "withheld_register_sha256": withheld, "cases": cases}
    _immutable(output_root / "batch-20-report.json", _json(report))
    return report

#!/usr/bin/env python3
"""Capture cutoff-safe SEC companyfacts/submissions packets for controlled Batch 34."""
...
CONFIG = ROOT / "backend/app/us_valuation/config/reset_batches_2026_08_14/batch_34.json"
VALUATION_DATE = "2026-08-14"


def manifest() -> list[SimpleNamespace]:
    value = json.loads(CONFIG.read_text())
    return [SimpleNamespace(**row) for row in value["members"]]
...
def capture_sources(*, output_root: Path, reuse_root: Path | None = None, user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in issuers}
    reused = [issuer.ticker for issuer in issuers if cached[issuer.ticker]]
    ...
    for issuer in issuers:
        ...
        _publish(output_root / issuer.ticker, _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=VALUATION_DATE, schema_version="FINSIGHT-BATCH-34-SOURCE-1"))
    after = {str(root): _tree_hash(root) for root in roots}
    if before != after:
        raise RuntimeError("serving changed")
    return {"manifest_count": len(issuers), "source_packet_count": len(issuers), "reused_tickers": reused, "fetched_tickers": fetched, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after}

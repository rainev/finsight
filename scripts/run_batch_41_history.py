#!/usr/bin/env python3
"""Stage Batch 41 private/public candidates without touching serving state."""
...
from app.us_valuation.batch_41_history import BATCH_41_HISTORY_VERSION, build_batch_41_history_result
...
def _public(issuer, result):
    ...
    value["public_assumptions"]["forecast_policy_version"] = BATCH_41_HISTORY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_41_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_41_HISTORY_VERSION
    value["issuer"]["classification_reason"] = "Frozen Batch 41 source, history, event and model review."
...
def run(*, source_root: Path, structural_root: Path, event_root: Path, output_root: Path, structural_cache_root: Path):
    ...
    for issuer in BATCH_41_MANIFEST:
        result = build_batch_41_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root, structural_cache_root=structural_cache_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-41-HISTORY-1", "batch": 41, "valuation_date": BATCH_41_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    report = {"schema_version": "FINSIGHT-BATCH-41-HISTORY-REPORT-1", "batch": 41, "valuation_date": BATCH_41_VALUATION_DATE, "policy_version": BATCH_41_HISTORY_VERSION, "denominator_tickers": list(BATCH_41_TICKERS), "attempted_count": 10, "pass_count": sum(row["outcome"] == "pass" for row in cases), "conditional_count": sum(row["outcome"] == "conditional" for row in cases), "withheld_count": sum(row["outcome"] == "withheld" for row in cases), "numeric_count": sum(row["outcome"] != "withheld" for row in cases), "pass_tickers": [row["ticker"] for row in cases if row["outcome"] == "pass"], "conditional_tickers": [row["ticker"] for row in cases if row["outcome"] == "conditional"], "withheld_tickers": [row["ticker"] for row in cases if row["outcome"] == "withheld"], "reliability_counts": reliability_counts, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_changed": False, "withheld_register_sha256": withheld_hash, "cases": cases}
    _immutable(output_root / "batch-41-report.json", _json(report))
    return report

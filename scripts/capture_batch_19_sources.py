#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 19 with cache-first reuse."""
...
def capture_sources(
    *,
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in BATCH_19_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_19_MANIFEST if cached[issuer.ticker]]
    missing = [issuer.ticker for issuer in BATCH_19_MANIFEST if not cached[issuer.ticker]]
    ...
    for issuer in BATCH_19_MANIFEST:
        ...
        packets[issuer.ticker] = _packet_payloads(
            ...
            schema_version="FINSIGHT-BATCH-19-SOURCE-1",
        )

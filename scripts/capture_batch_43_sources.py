#!/usr/bin/env python3
"""Capture cutoff-safe SEC Companyfacts/submissions packets for Batch 43."""
...
def capture_sources(*, output_root: Path, reuse_roots=(), user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {issuer.ticker: next((value for reuse_root in candidates if (value := _reuse(reuse_root, issuer.ticker)) is not None), None) for issuer in BATCH_43_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_43_MANIFEST if cached[issuer.ticker]]
    ...
    for issuer in BATCH_43_MANIFEST:
        ...
        _publish(output_root / issuer.ticker, _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_43_VALUATION_DATE, schema_version="FINSIGHT-BATCH-43-SOURCE-1"))

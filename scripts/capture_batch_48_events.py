#!/usr/bin/env python3
"""Capture Batch 48 cutoff-event filings and exhibits."""
...
def capture_events(*, source_root: Path, output_root: Path, user_agent: str | None = None) -> dict:
    ...
    for issuer in BATCH_48_MANIFEST:
        ...
        for index, filed in enumerate(recent["filingDate"]):
            ...
            if not ("2026-06-30" <= filed <= BATCH_48_VALUATION_DATE and form in EVENT_FORMS):
                continue
            ...
            for link in sorted(set(re.findall(r'href=["\x27]([^"\x27]+)', raw.decode(errors="ignore"), re.I))):
                low = link.lower()
                if "/" in link or not low.endswith((".htm", ".html")) or not any(token in low for token in ("99", "exhibit", "release", "earnings", "ex10", "ex4", "presentation", "livemaster")):
                    continue
                documents.append((link, client.filing_attachment(issuer.cik, accession, link)))
            captured = []
            for name, data in documents[:20]:
    ...
    summary = {"schema_version": "FINSIGHT-BATCH-48-EVENT-SCREEN-1", "valuation_date": BATCH_48_VALUATION_DATE, "attempted": 10, "cases": cases, "serving_artifacts_changed": False}

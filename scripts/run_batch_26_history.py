...
from app.us_valuation.batch_26_history import BATCH_26_HISTORY_VERSION,FORECAST_YEARS,build_batch_26_history_result
...
def _public(i,r):
 v=_prior_public(i,r);v['issuer']['classification_reason']='Frozen Batch 26 technology source and history review.';v['public_assumptions']['forecast_policy_version']=BATCH_26_HISTORY_VERSION;v['forecast_quality']['policy_version']=BATCH_26_HISTORY_VERSION;v['methodology']['forecast_policy']=BATCH_26_HISTORY_VERSION;v['model_policy']['reason']=r['warning'];v['public_assumptions']['forecast_mode']='normalized_equity_earnings' if i.ticker=='IBM' else 'history_backed_normalized_cash_conversion';v['public_assumptions']['forecast_years']=5 if i.ticker=='IBM' else FORECAST_YEARS;v=sanitize_public_artifact(v)
...
def run(*,source_root:Path,structural_root:Path,output_root:Path):
 ...
 report={'schema_version':'FINSIGHT-BATCH-26-HISTORY-REPORT-1','batch':26,'valuation_date':BATCH_26_VALUATION_DATE,'policy_version':BATCH_26_HISTORY_VERSION,'denominator_tickers':list(BATCH_26_TICKERS),'attempted_count':10,'pass_count':sum(x['outcome']=='pass' for x in cases),'conditional_count':sum(x['outcome']=='conditional' for x in cases),'withheld_count':sum(x['outcome']=='withheld' for x in cases),'numeric_count':sum(x['outcome']!='withheld' for x in cases),'pass_tickers':[x['ticker'] for x in cases if x['outcome']=='pass'],'conditional_tickers':[x['ticker'] for x in cases if x['outcome']=='conditional'],'withheld_tickers':[x['ticker'] for x in cases if x['outcome']=='withheld'],'reliability_counts':rel,'serving_artifacts_changed':False,'serving_hash_before':before,'serving_hash_after':after,'watchlist_changed':False,'watchlist_sha256':watch,'withheld_register_changed':False,'withheld_register_sha256':withheld,'cases':cases};_immutable(output_root/'batch-26-report.json',_json(report));return report
def main():

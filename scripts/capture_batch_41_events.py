#!/usr/bin/env python3
"""Capture Batch 41 cutoff-event filings and candidate exhibits for source review."""
...
def capture_events(*, source_root: Path, output_root: Path, user_agent: str | None = None) -> dict:
    ...
    for issuer in BATCH_41_MANIFEST:
        ...
        for index, filed in enumerate(recent["filingDate"]):
            ...
            if not ("2026-06-30" <= filed <= BATCH_41_VALUATION_DATE and form in EVENT_FORMS):
                continue
            ...
            for link in links:
                ...
                if "/" in link or not lowered.endswith((".htm", ".html")):
                    continue
                if not any(token in lowered for token in ("99", "exhibit", "release", "ex10", "ex4", "livemaster")):
                    continue
    ...
    summary = {"schema_version": "FINSIGHT-BATCH-41-EVENT-SCREEN-1", "valuation_date": BATCH_41_VALUATION_DATE, "attempted": 10, "cases": cases, "serving_artifacts_changed": False}

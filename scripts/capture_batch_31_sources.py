...
def capture_sources(*, output_root: Path, reuse_root: Path | None = None, user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in BATCH_31_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_31_MANIFEST if cached[issuer.ticker]]
    ...
    for issuer in BATCH_31_MANIFEST:
        ...
        _publish(output_root / issuer.ticker, _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_31_VALUATION_DATE, schema_version="FINSIGHT-BATCH-31-SOURCE-1"))

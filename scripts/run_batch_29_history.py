...
from app.us_valuation.batch_29 import BATCH_29_MANIFEST, BATCH_29_TICKERS, BATCH_29_VALUATION_DATE
from app.us_valuation.batch_29_history import BATCH_29_HISTORY_VERSION, FORECAST_YEARS, build_batch_29_history_result
...
def _public(issuer, result):
    ...
    value["issuer"]["classification_reason"] = "Frozen Batch 29 technology source and history review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_29_HISTORY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_29_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_29_HISTORY_VERSION
...
def run(*, source_root: Path, structural_root: Path, output_root: Path):
    ...
    for issuer in BATCH_29_MANIFEST:
        result = build_batch_29_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-29-HISTORY-1", "batch": 29, "valuation_date": BATCH_29_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    report = {
        "schema_version": "FINSIGHT-BATCH-29-HISTORY-REPORT-1",
        "batch": 29,
        "valuation_date": BATCH_29_VALUATION_DATE,
        "policy_version": BATCH_29_HISTORY_VERSION,
        "denominator_tickers": list(BATCH_29_TICKERS),
        "attempted_count": 10,
        "pass_count": sum(case["outcome"] == "pass" for case in cases),
        "conditional_count": sum(case["outcome"] == "conditional" for case in cases),
        "withheld_count": sum(case["outcome"] == "withheld" for case in cases),
        "numeric_count": sum(case["outcome"] != "withheld" for case in cases),
        "pass_tickers": [case["ticker"] for case in cases if case["outcome"] == "pass"],
        "conditional_tickers": [case["ticker"] for case in cases if case["outcome"] == "conditional"],
        "withheld_tickers": [case["ticker"] for case in cases if case["outcome"] == "withheld"],
    ...
    _immutable(output_root / "batch-29-report.json", _json(report))
    return report

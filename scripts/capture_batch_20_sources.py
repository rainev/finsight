#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 20 with cache-first reuse."""
...
def capture_sources(*, output_root: Path, reuse_root: Path | None = None, user_agent: str | None = None, refresh: bool = False, client: Any = None, protected_serving_roots=PROTECTED_ROOTS):
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in BATCH_20_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_20_MANIFEST if cached[issuer.ticker]]
    missing = [issuer.ticker for issuer in BATCH_20_MANIFEST if not cached[issuer.ticker]]
    ...
    for issuer in BATCH_20_MANIFEST:
        ...
        if packet:
            submissions, facts, submissions_meta, facts_meta = packet
        else:
        ...
        packets[issuer.ticker] = _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_20_VALUATION_DATE, schema_version="FINSIGHT-BATCH-20-SOURCE-1")

#!/usr/bin/env python3
"""Capture cutoff-safe SEC Companyfacts/submissions packets for Batch 45."""
...
def capture_sources(*,output_root:Path,reuse_roots=(),user_agent:str|None=None,refresh:bool=False,client=None,protected_serving_roots=PROTECTED_ROOTS)->dict:
    output_root=Path(output_root);roots=tuple(map(Path,protected_serving_roots));candidates=tuple(map(Path,reuse_roots));_assert_non_serving(output_root,roots);before={str(r):_tree_hash(r) for r in roots};cached={i.ticker:next((v for rr in candidates if (v:=_reuse(rr,i.ticker)) is not None),None) for i in BATCH_45_MANIFEST};reused=[i.ticker for i in BATCH_45_MANIFEST if cached[i.ticker]];client=client or SecClient(user_agent=user_agent,cache_dir=output_root.parent/".sec-cache");fetched=[]
    for i in BATCH_45_MANIFEST:
        ...
        try:
            payloads=_packet_payloads(i,submissions,facts,submissions_metadata=sm,facts_metadata=fm,valuation_date=BATCH_45_VALUATION_DATE,schema_version="FINSIGHT-BATCH-45-SOURCE-1")
            ...
            payloads=_packet_payloads(i,patched,facts,submissions_metadata=sm,facts_metadata=fm,valuation_date=BATCH_45_VALUATION_DATE,schema_version="FINSIGHT-BATCH-45-SOURCE-1")

#!/usr/bin/env python3
"""Capture cutoff-safe SEC Companyfacts/submissions packets for Batch 40."""
...
def capture_sources(*,output_root:Path,reuse_root:Path|None=None,user_agent:str|None=None,refresh:bool=False,client=None,protected_serving_roots=PROTECTED_ROOTS)->dict:
 output_root=Path(output_root);roots=tuple(map(Path,protected_serving_roots));_assert_non_serving(output_root,roots);before={str(r):_tree_hash(r) for r in roots};cached={i.ticker:_reuse(reuse_root,i.ticker) for i in BATCH_40_MANIFEST};reused=[i.ticker for i in BATCH_40_MANIFEST if cached[i.ticker]];client=client or SecClient(user_agent=user_agent,cache_dir=output_root.parent/'.sec-cache');fetched=[]
 for i in BATCH_40_MANIFEST:
  ...
  _publish(output_root/i.ticker,_packet_payloads(i,sub,facts,submissions_metadata=sub_meta,facts_metadata=facts_meta,valuation_date=BATCH_40_VALUATION_DATE,schema_version='FINSIGHT-BATCH-40-SOURCE-1'))

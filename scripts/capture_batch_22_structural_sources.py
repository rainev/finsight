...
from app.us_valuation.batch_22 import BATCH_22_MANIFEST,BATCH_22_VALUATION_DATE
...
def control(root,i):
 p=Path(root)/i.ticker;m=json.loads((p/'source-manifest.json').read_text());eligible=[r for r in m['eligible_filings'] if r['form'] in ELIGIBLE_FORMS]
 # HON's 2026-07-24 10-Q/A only amends the cover-page share count.  The
 # complete 2026-06-30 statements remain in the underlying 2026-07-23 10-Q.
 # Parse that full filing structurally; the amendment is retained separately
 # in the SEC packet and is used only for its updated share fact downstream.
 if i.ticker=='HON':eligible=[r for r in eligible if r['form']=='10-Q']
 f=dict(max(eligible,key=lambda r:(r['filed'],r['accession'])));s=json.loads((p/'submissions.json').read_text())['filings']['recent'];x=s['accessionNumber'].index(f['accession']);f['report_date']=s['reportDate'][x];return f,m
def capture_structural_sources(*,source_root:Path,output_root:Path,cache_root:Path,user_agent=None,refresh=False,client=None,package_capture=None,parse=None,reuse_roots=(),protected_serving_roots=PROTECTED_ROOTS):
 source_root,output_root,cache_root=map(Path,(source_root,output_root,cache_root));_validate_paths(source_root,output_root,cache_root,protected_serving_roots);before={str(r):_tree_hash(r) for r in protected_serving_roots};ctl={i.ticker:control(source_root,i) for i in BATCH_22_MANIFEST};cached={i.ticker:_reuse_any_structural(tuple(map(Path,reuse_roots)),i,ctl[i.ticker][0]) for i in BATCH_22_MANIFEST}

#!/usr/bin/env python3
"""Capture parsed controlling filings for Batch 16 with cache-first reuse."""
...
def _control(root, issuer):
    ...
    if manifest.get("schema_version") != "FINSIGHT-BATCH-16-SOURCE-1" or manifest.get("issuer") != {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}:
...
def capture_structural_sources(*, source_root: Path, output_root: Path, cache_root: Path, user_agent: str | None = None, refresh: bool = False, client: Any = None, package_capture: Callable[..., Path] | None = None, parse: Callable[..., Any] | None = None, reuse_roots: tuple[Path, ...] = (), protected_serving_roots=PROTECTED_ROOTS):
    ...
    controls = {issuer.ticker: _control(source_root, issuer) for issuer in BATCH_16_MANIFEST}
    cached = {issuer.ticker: _reuse_any_structural(reuse_roots, issuer, controls[issuer.ticker][0]) for issuer in BATCH_16_MANIFEST}
    ...
    summary = {"valuation_date": BATCH_16_VALUATION_DATE, "attempted": 10, "parsed": 10, "failed": 0, "reused_tickers": reused, "captured_tickers": captured, "cases": cases, "serving_hash_before": before, "serving_hash_after": after, "serving_artifacts_changed": False}

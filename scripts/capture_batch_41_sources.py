#!/usr/bin/env python3
"""Capture cutoff-safe SEC Companyfacts/submissions packets for Batch 41."""
...
def capture_sources(*, output_root: Path, reuse_roots=(), user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {
        issuer.ticker: next((value for reuse_root in candidates if (value := _reuse(reuse_root, issuer.ticker)) is not None), None)
        for issuer in BATCH_41_MANIFEST
    }
    reused = [issuer.ticker for issuer in BATCH_41_MANIFEST if cached[issuer.ticker]]
    ...
    for issuer in BATCH_41_MANIFEST:
        ...
        _publish(
            ...
            _packet_payloads(
                ...
                schema_version="FINSIGHT-BATCH-41-SOURCE-1",
            ),
        )

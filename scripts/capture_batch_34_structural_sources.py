#!/usr/bin/env python3
"""Capture and parse controlling SEC XBRL filings for controlled Batch 34."""
...
from capture_batch_34_sources import CONFIG, VALUATION_DATE, manifest


def control(root: Path, issuer: SimpleNamespace) -> tuple[dict, dict]:
...
def capture_structural_sources(*, source_root: Path, output_root: Path, cache_root: Path, user_agent: str | None = None, refresh: bool = False, client=None, package_capture=None, parse=None, reuse_roots=(), protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    controlling = {issuer.ticker: control(source_root, issuer) for issuer in issuers}
    cached = {issuer.ticker: _reuse_any_structural(tuple(map(Path, reuse_roots)), issuer, controlling[issuer.ticker][0]) for issuer in issuers}
    ...
    summary = {"valuation_date": VALUATION_DATE, "attempted": len(issuers), "parsed": len(issuers), "failed": 0, "reused_tickers": reused, "captured_tickers": captured, "cases": cases, "serving_hash_before": before, "serving_hash_after": after, "serving_artifacts_changed": False}

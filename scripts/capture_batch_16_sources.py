#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 16, reusing cached packets first."""
...
def capture_sources(*, output_root: Path, reuse_root: Path | None = None, user_agent: str | None = None, refresh: bool = False, client: Any = None, protected_serving_roots=PROTECTED_ROOTS):
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in BATCH_16_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_16_MANIFEST if cached[issuer.ticker]]
    missing = [issuer.ticker for issuer in BATCH_16_MANIFEST if not cached[issuer.ticker]]
    ...
    for issuer in BATCH_16_MANIFEST:
        ...
        packets[issuer.ticker] = _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_metadata, facts_metadata=facts_metadata, valuation_date=BATCH_16_VALUATION_DATE, schema_version="FINSIGHT-BATCH-16-SOURCE-1")

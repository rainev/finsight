#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 12, reusing all cached packets first."""
...
def capture_sources(
    *,
    ...
    reused = [issuer.ticker for issuer in BATCH_12_MANIFEST if cached[issuer.ticker]]
    missing = [issuer.ticker for issuer in BATCH_12_MANIFEST if not cached[issuer.ticker]]
    client = client or SecClient(
        user_agent=user_agent, cache_dir=output_root.parent / ".sec-cache"
    )
    packets = {}
    fetched = []
    for issuer in BATCH_12_MANIFEST:
        ...
        packets[issuer.ticker] = _packet_payloads(
            ...
            schema_version="FINSIGHT-BATCH-12-SOURCE-1",
        )

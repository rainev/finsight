#!/usr/bin/env python3
"""Stage the authorized EXE/ALB recovery attempt without bookkeeping writes."""
...
from app.us_valuation.batch_42_recovery import BATCH_42_RECOVERY_VERSION, build_batch_42_recovery_result
...
from run_batch_42_history import _public as _initial_public


APPROVED_INITIAL_REPORT_SHA256 = "c56561c2414cb8a44d4230720a303cd508b2869667c825efec671cb54c5e16f1"
...
def _public(issuer, result):
    ...
    value["public_assumptions"]["forecast_policy_version"] = BATCH_42_RECOVERY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_42_RECOVERY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_42_RECOVERY_VERSION
    value["issuer"]["classification_reason"] = "Frozen Batch 42 sources plus one controlled EXE/ALB recovery attempt."
    value["model_policy"]["reason"] = result["warning"]
    if issuer.ticker in {"EXE", "ALB"}:
        value["public_assumptions"]["forecast_mode"] = "withheld_after_resource_cycle_recovery_attempt"
...
def run(*, initial_root: Path, source_root: Path, structural_root: Path, event_root: Path, output_root: Path, structural_cache_root: Path):
    initial_report_path = Path(initial_root) / "batch-42-report.json"
    ...
    if _sha(initial_report_path) != APPROVED_INITIAL_REPORT_SHA256 or (initial_report["pass_count"], initial_report["conditional_count"], initial_report["withheld_count"]) != (0, 8, 2):
        raise ValueError("verified Batch 42 initial result required")
    ...
    for issuer in BATCH_42_MANIFEST:
        if issuer.ticker not in {"EXE", "ALB"}:
            ...
            continue
        result = build_batch_42_recovery_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root, structural_cache_root=structural_cache_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-42-RECOVERY-1", "batch": 42, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik}, "recovery": result, "controlled_outcome": case}
    ...
    report = {"schema_version": "FINSIGHT-BATCH-42-RECOVERY-REPORT-1", "batch": 42, "attempted_count": 2, "attempted_tickers": ["EXE", "ALB"], "recovered_to_conditional_tickers": [], "still_withheld_tickers": ["EXE", "ALB"], "pass_count": sum(row["outcome"] == "pass" for row in cases), "conditional_count": sum(row["outcome"] == "conditional" for row in cases), "withheld_count": sum(row["outcome"] == "withheld" for row in cases), "numeric_count": sum(row["outcome"] != "withheld" for row in cases), "serving_artifacts_changed": False, "watchlist_changed": False, "withheld_register_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_sha256": withheld_hash, "cases": cases}
    _immutable(output_root / "batch-42-recovery-report.json", _json(report))
    return report

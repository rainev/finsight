#!/usr/bin/env python3
"""Capture cutoff-safe SEC Companyfacts/submissions packets for Batch 36."""
...
def capture_sources(*, output_root: Path, reuse_root: Path | None = None, user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in BATCH_36_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_36_MANIFEST if cached[issuer.ticker]]
    ...
    for issuer in BATCH_36_MANIFEST:
        ...
        payloads = _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_36_VALUATION_DATE, schema_version="FINSIGHT-BATCH-36-SOURCE-1")

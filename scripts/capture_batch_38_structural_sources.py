#!/usr/bin/env python3
"""Capture and parse controlling SEC XBRL filings for Batch 38."""
...
def capture_structural_sources(*,source_root:Path,output_root:Path,cache_root:Path,user_agent:str|None=None,refresh:bool=False,client=None,package_capture=None,parse=None,reuse_roots=(),protected_serving_roots=PROTECTED_ROOTS):
 source_root,output_root,cache_root=map(Path,(source_root,output_root,cache_root));_validate_paths(source_root,output_root,cache_root,protected_serving_roots);before={str(r):_tree_hash(r) for r in protected_serving_roots};controlling={i.ticker:control(source_root,i) for i in BATCH_38_MANIFEST};cached={i.ticker:_reuse_any_structural(tuple(map(Path,reuse_roots)),i,controlling[i.ticker][0]) for i in BATCH_38_MANIFEST}

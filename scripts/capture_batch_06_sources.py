#!/usr/bin/env python3
"""Capture immutable SEC packets for frozen Batch 06."""
...
def capture_sources(*,output_root:Path,reuse_root:Path|None=None,user_agent:str|None=None,refresh:bool=False,client:Any=None,protected_serving_roots=PROTECTED_ROOTS):
 ...
 for issuer in BATCH_06_MANIFEST:
  ...
  packets[issuer.ticker]=_packet_payloads(issuer,sub,facts,submissions_metadata=sm,facts_metadata=fm,valuation_date=BATCH_06_VALUATION_DATE,schema_version='FINSIGHT-BATCH-06-SOURCE-1')
 for ticker,payloads in packets.items():_publish(output_root/ticker,payloads)

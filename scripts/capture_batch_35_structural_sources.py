#!/usr/bin/env python3
"""Capture and parse controlling SEC XBRL filings for controlled Batch 35."""
...
def capture_structural_sources(*, source_root: Path, output_root: Path, cache_root: Path, user_agent: str | None = None, refresh: bool = False, client=None, package_capture=None, parse=None, reuse_roots=(), protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    controlling = {issuer.ticker: control(source_root, issuer) for issuer in BATCH_35_MANIFEST}
    cached = {issuer.ticker: _reuse_any_structural(tuple(map(Path, reuse_roots)), issuer, controlling[issuer.ticker][0]) for issuer in BATCH_35_MANIFEST}
    ...
    summary = {"schema_version": "FINSIGHT-BATCH-35-STRUCTURAL-SOURCE-1", "valuation_date": BATCH_35_VALUATION_DATE, "attempted": len(BATCH_35_MANIFEST), "parsed": len(BATCH_35_MANIFEST), "failed": 0, "reused_tickers": reused, "captured_tickers": captured, "cases": cases, "serving_hash_before": before, "serving_hash_after": after, "serving_artifacts_changed": False}

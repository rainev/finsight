...
from app.us_valuation.batch_32_history import BATCH_32_HISTORY_VERSION, FORECAST_YEARS, build_batch_32_history_result
...
def _public(issuer, result):
    ...
    value["issuer"]["classification_reason"] = "Frozen Batch 32 technology source, event, and history review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_32_HISTORY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_32_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_32_HISTORY_VERSION
    value["model_policy"]["reason"] = result["warning"]
    value["public_assumptions"]["forecast_mode"] = "history_backed_bounded_uncertainty"
...
def run(*, source_root: Path, structural_root: Path, event_root: Path, output_root: Path):
    ...
    for issuer in BATCH_32_MANIFEST:
        result = build_batch_32_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-32-HISTORY-1", "batch": 32, "valuation_date": BATCH_32_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    report = {"schema_version": "FINSIGHT-BATCH-32-HISTORY-REPORT-1", "batch": 32, "valuation_date": BATCH_32_VALUATION_DATE, "policy_version": BATCH_32_HISTORY_VERSION, "denominator_tickers": list(BATCH_32_TICKERS), "attempted_count": 10, "pass_count": sum(case["outcome"] == "pass" for case in cases), "conditional_count": sum(case["outcome"] == "conditional" for case in cases), "withheld_count": sum(case["outcome"] == "withheld" for case in cases), "numeric_count": sum(case["outcome"] != "withheld" for case in cases), "pass_tickers": [case["ticker"] for case in cases if case["outcome"] == "pass"], "conditional_tickers": [case["ticker"] for case in cases if case["outcome"] == "conditional"], "withheld_tickers": [case["ticker"] for case in cases if case["outcome"] == "withheld"], "reliability_counts": reliability_counts, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_changed": False, "withheld_register_sha256": withheld_hash, "cases": cases}
    _immutable(output_root / "batch-32-report.json", _json(report))
    return report

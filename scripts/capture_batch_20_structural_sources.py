#!/usr/bin/env python3
"""Capture parsed controlling filings for Batch 20 with cache-first reuse."""
...
from app.us_valuation.batch_20 import BATCH_20_MANIFEST, BATCH_20_VALUATION_DATE
...
def _control(root: Path, issuer):
    ...
    if manifest.get("schema_version") != "FINSIGHT-BATCH-20-SOURCE-1" or manifest.get("issuer") != {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}:
    ...
    indexes = [index for index, accession in enumerate(recent["accessionNumber"]) if accession == filing["accession"]]
    if len(indexes) != 1:
        raise ValueError(f"{issuer.ticker}: controlling report date unresolved")
    index = indexes[0]
    if recent["filingDate"][index] != filing["filed"] or recent["form"][index] != filing["form"] or recent["primaryDocument"][index] != filing["primary_document"]:
...
def capture_structural_sources(*, source_root: Path, output_root: Path, cache_root: Path, user_agent: str | None = None, refresh: bool = False, client: Any = None, package_capture: Callable[..., Path] | None = None, parse: Callable[..., Any] | None = None, reuse_roots: tuple[Path, ...] = (), protected_serving_roots=PROTECTED_ROOTS):
    ...
    controls = {issuer.ticker: _control(source_root, issuer) for issuer in BATCH_20_MANIFEST}
    cached = {issuer.ticker: _reuse_any_structural(reuse_roots, issuer, controls[issuer.ticker][0]) for issuer in BATCH_20_MANIFEST}
    ...
    summary = {"valuation_date": BATCH_20_VALUATION_DATE, "attempted": 10, "parsed": 10, "failed": 0, "reused_tickers": reused, "captured_tickers": captured, "cases": cases, "serving_hash_before": before, "serving_hash_after": after, "serving_artifacts_changed": False}

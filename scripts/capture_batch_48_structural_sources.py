#!/usr/bin/env python3
"""Capture and parse controlling SEC XBRL filings for Batch 48."""
...
def capture_structural_sources(*, source_root: Path, output_root: Path, cache_root: Path, user_agent: str | None = None, refresh: bool = False, client=None, package_capture=None, parse=None, reuse_roots=(), protected_serving_roots=PROTECTED_ROOTS, parse_timeout_seconds: int = 300):
    ...
    controls = {issuer.ticker: control(source_root, issuer) for issuer in BATCH_48_MANIFEST}
    cached = {issuer.ticker: _reuse_any_structural(tuple(map(Path, reuse_roots)), issuer, controls[issuer.ticker][0]) for issuer in BATCH_48_MANIFEST}
    if any(value is None for value in cached.values()) and (client is None or package_capture is None or parse is None):
        ...
        package_capture = package_capture or pack
        parse = parse or parser
    ...
    summary = {"schema_version": "FINSIGHT-BATCH-48-STRUCTURAL-SOURCE-1", "valuation_date": BATCH_48_VALUATION_DATE, "attempted": 10, "parsed": 10, "failed": 0, "reused_tickers": reused, "captured_tickers": captured, "cases": cases, "serving_hash_before": before, "serving_hash_after": after, "serving_artifacts_changed": False}
...
def main() -> None:
    ...
    parser.add_argument("--parse-timeout-seconds", type=int, default=300)

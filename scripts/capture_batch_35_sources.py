#!/usr/bin/env python3
"""Capture cutoff-safe SEC companyfacts/submissions packets for controlled Batch 35."""
...
def capture_sources(*, output_root: Path, reuse_root: Path | None = None, user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {issuer.ticker: _reuse(reuse_root, issuer.ticker) for issuer in BATCH_35_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_35_MANIFEST if cached[issuer.ticker]]
    ...
    for issuer in BATCH_35_MANIFEST:
        ...
        _publish(output_root / issuer.ticker, _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_35_VALUATION_DATE, schema_version="FINSIGHT-BATCH-35-SOURCE-1"))
    after = {str(root): _tree_hash(root) for root in roots}
    if before != after:
        raise RuntimeError("serving changed")
    return {"manifest_count": len(BATCH_35_MANIFEST), "source_packet_count": len(BATCH_35_MANIFEST), "reused_tickers": reused, "fetched_tickers": fetched, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after}

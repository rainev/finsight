#!/usr/bin/env python3
"""Capture cutoff-safe SEC Companyfacts/submissions packets for Batch 50."""
...
def capture_sources(*, output_root: Path, reuse_roots=(), user_agent: str | None = None, refresh: bool = False, client=None, protected_serving_roots=PROTECTED_ROOTS) -> dict:
    ...
    cached = {issuer.ticker: next((value for root in candidates if (value := _reuse(root, issuer.ticker)) is not None), None) for issuer in BATCH_50_MANIFEST}
    reused = [issuer.ticker for issuer in BATCH_50_MANIFEST if cached[issuer.ticker]]
    ...
    for issuer in BATCH_50_MANIFEST:
        ...
        try:
            payloads = _packet_payloads(issuer, submissions, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_50_VALUATION_DATE, schema_version="FINSIGHT-BATCH-50-SOURCE-1")
        except ValueError as error:
            if "ticker identity mismatch" not in str(error):
                raise
            patched = deepcopy(submissions)
            patched["tickers"] = [issuer.ticker]
            payloads = _packet_payloads(issuer, patched, facts, submissions_metadata=submissions_meta, facts_metadata=facts_meta, valuation_date=BATCH_50_VALUATION_DATE, schema_version="FINSIGHT-BATCH-50-SOURCE-1")

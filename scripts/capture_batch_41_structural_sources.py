#!/usr/bin/env python3
"""Capture and parse controlling SEC XBRL filings for Batch 41."""
...
from app.us_valuation.batch_41 import BATCH_41_MANIFEST, BATCH_41_VALUATION_DATE
...
def capture_structural_sources(*, source_root: Path, output_root: Path, cache_root: Path, user_agent: str | None = None, refresh: bool = False, client=None, package_capture=None, parse=None, reuse_roots=(), protected_serving_roots=PROTECTED_ROOTS):
    ...
    controlling = {issuer.ticker: control(source_root, issuer) for issuer in BATCH_41_MANIFEST}
    cached = {issuer.ticker: _reuse_any_structural(tuple(map(Path, reuse_roots)), issuer, controlling[issuer.ticker][0]) for issuer in BATCH_41_MANIFEST}
    ...
    for issuer in BATCH_41_MANIFEST:
        ...
        receipt = {
            "schema_version": "FINSIGHT-BATCH-41-STRUCTURAL-SOURCE-1",
            "valuation_date": BATCH_41_VALUATION_DATE,
            "ticker": issuer.ticker,
            "cik": issuer.cik,
            "filing": filing,
            "source_packet_manifest_sha256": _sha256(_json_bytes(packet)),
    ...
    summary = {"schema_version": "FINSIGHT-BATCH-41-STRUCTURAL-SOURCE-1", "valuation_date": BATCH_41_VALUATION_DATE, "attempted": 10, "parsed": 10, "failed": 0, "reused_tickers": reused, "captured_tickers": captured, "cases": cases, "serving_hash_before": before, "serving_hash_after": after, "serving_artifacts_changed": False}

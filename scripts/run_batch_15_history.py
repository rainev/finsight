#!/usr/bin/env python3
"""Stage initial Batch 15 historical/practical outcomes without protected-state writes."""
...
from app.us_valuation.batch_15_history import BATCH_15_HISTORY_VERSION, build_batch_15_history_result
...
def _public(issuer, result):
    ...
    value["issuer"]["classification_reason"] = "Frozen Batch 15 Health Care lane plus issuer-specific source, history, claim, and event review."
    value["public_assumptions"]["forecast_policy_version"] = BATCH_15_HISTORY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_15_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_15_HISTORY_VERSION
    value["model_policy"]["reason"] = result["warning"]
    if result["availability_type"] == "not_available":
        modes = {
            "LH": "unavailable_unbounded_doj_settlement_claim",
            "ISRG": "unavailable_unbounded_product_liability_claims",
            "ALGN": "unavailable_unbounded_legal_competition_claims",
        }
...
def run(*, source_root: Path, structural_root: Path, output_root: Path):
    ...
    for issuer in BATCH_15_MANIFEST:
        result = build_batch_15_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-15-HISTORY-1", "batch": 15, "valuation_date": BATCH_15_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    if before != after or watchlist_hash != watchlist_after or withheld_hash != withheld_after:
        raise RuntimeError("Batch 15 changed protected or cumulative state")
    report = {"schema_version": "FINSIGHT-BATCH-15-HISTORY-REPORT-1", "batch": 15, "valuation_date": BATCH_15_VALUATION_DATE, "policy_version": BATCH_15_HISTORY_VERSION, "denominator_tickers": list(BATCH_15_TICKERS), "attempted_count": 10, "pass_count": sum(row["outcome"] == "pass" for row in cases), "conditional_count": sum(row["outcome"] == "conditional" for row in cases), "withheld_count": sum(row["outcome"] == "withheld" for row in cases), "numeric_count": sum(row["outcome"] != "withheld" for row in cases), "pass_tickers": [row["ticker"] for row in cases if row["outcome"] == "pass"], "conditional_tickers": [row["ticker"] for row in cases if row["outcome"] == "conditional"], "withheld_tickers": [row["ticker"] for row in cases if row["outcome"] == "withheld"], "reliability_counts": reliability_counts, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_changed": False, "withheld_register_sha256": withheld_hash, "cases": cases}
    _immutable(output_root / "batch-15-report.json", _json(report))
    return report

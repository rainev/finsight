#!/usr/bin/env python3
"""Stage history-backed Batch 06 Pass repairs without serving or watchlist writes."""
...
from app.us_valuation.batch_06_launch_first import BATCH_06_HISTORY_REPAIR_VERSION,PASS_TICKERS,build_batch_06_launch_first_result
from run_batch_06_launch_first import PROTECTED,WATCHLIST,_immutable,_json,_public,_tree
...
def _pass_public(issuer,result):
 value=_public(issuer,result);scenario=result['scenario_range'];base=scenario['base'];value['primary_valuation_method']='fcff_dcf';value['model_policy']={'primary':'fcff_dcf','supporting':[],'blend_models':False,'reason':'History-backed source-bounded normalized cash-FCFF with a completed current claims bridge.'};value['models']={'fcff_dcf':{'model':'fcff_dcf','output_type':'intrinsic_value_per_share','currency':'USD','intrinsic_value_per_share':base,'publication_state':'review_required','errors':[],'warnings':['History-backed source-bounded estimate; ordinary scenario uncertainty remains.']}};value['scenario_range']={**scenario,'label':'assumption range, not a statistical confidence interval'};value['bridge_quality']={'blocking_fields':[],'bounded_fields':[],'complete':True,'decision':'complete','intrinsic_value_range':{'low':base,'midpoint':base,'high':base,'spread_ratio':0.0,'spread_limit':.01},'reason_codes':[],'usable':True};value['forecast_quality']['warnings']=['History-backed normalized cash-FCFF; scenario assumptions require review.'];value['review']['warnings']=['History-backed source-bounded estimate. Ordinary business and scenario uncertainty may materially affect the range.'];value['reliability']=result['history_reliability'];value['confidence']={'label':result['history_reliability']['label'],'reasons':result['history_reliability']['reasons']};public=sanitize_public_artifact(value)
...
def _history_public(issuer,result):
 ...
 public['reliability']=result['history_reliability'];public['confidence']={'label':result['history_reliability']['label'],'reasons':result['history_reliability']['reasons']};public['forecast_quality']['policy_version']=BATCH_06_HISTORY_REPAIR_VERSION;public['methodology']['forecast_policy']=BATCH_06_HISTORY_REPAIR_VERSION;value=sanitize_public_artifact(public);expected='available' if issuer.ticker in PASS_TICKERS else 'conditional_estimate'

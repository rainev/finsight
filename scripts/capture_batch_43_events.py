#!/usr/bin/env python3
"""Capture Batch 43 cutoff-event filings and candidate exhibits for source review."""
...
def capture_events(*, source_root: Path, output_root: Path, user_agent: str | None = None) -> dict:
    ...
    for issuer in BATCH_43_MANIFEST:
        ...
        for index, filed in enumerate(recent["filingDate"]):
            ...
            if not ("2026-06-30" <= filed <= BATCH_43_VALUATION_DATE and form in EVENT_FORMS):
                continue
            ...
            for link in sorted(set(re.findall(r'href=["\x27]([^"\x27]+)', raw.decode(errors="ignore"), re.I))):
                lowered = link.lower()
                if "/" in link or not lowered.endswith((".htm", ".html")) or not any(token in lowered for token in ("99", "exhibit", "release", "earnings", "ex10", "ex4", "presentation", "livemaster")):
                    continue
    ...
    summary = {"schema_version": "FINSIGHT-BATCH-43-EVENT-SCREEN-1", "valuation_date": BATCH_43_VALUATION_DATE, "attempted": 10, "cases": cases, "serving_artifacts_changed": False}

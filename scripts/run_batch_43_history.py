#!/usr/bin/env python3
"""Stage Batch 43 private/public candidates without touching confirmation state."""
...
from app.us_valuation.batch_43_history import BATCH_43_HISTORY_VERSION, build_batch_43_history_result
...
from run_batch_42_history import _public as _prior_public


def _public(issuer, result):
    value = _prior_public(issuer, result)
    value["public_assumptions"]["forecast_policy_version"] = BATCH_43_HISTORY_VERSION
    value["forecast_quality"]["policy_version"] = BATCH_43_HISTORY_VERSION
    value["methodology"]["forecast_policy"] = BATCH_43_HISTORY_VERSION
    value["issuer"]["classification_reason"] = "Frozen Batch 43 source, history, event and model review."
    value["model_policy"]["reason"] = result["warning"]
    value["public_assumptions"]["share_count_basis"] = "The calculator's share fields are governed scenario denominators anchored to the latest reported common shares; the +/-1.5% range is a disclosed dilution stress."
    if result["availability_type"] == "not_available":
        value["public_assumptions"]["forecast_mode"] = "unavailable_current_company_scope"
...
def run(*, source_root: Path, structural_root: Path, event_root: Path, output_root: Path, structural_cache_root: Path, cop_annual_root: Path):
    ...
    for issuer in BATCH_43_MANIFEST:
        result = build_batch_43_history_result(ticker=issuer.ticker, source_root=source_root, structural_root=structural_root, event_root=event_root, structural_cache_root=structural_cache_root, cop_annual_root=cop_annual_root)
        ...
        private = {"schema_version": "FINSIGHT-BATCH-43-HISTORY-1", "batch": 43, "valuation_date": BATCH_43_VALUATION_DATE, "issuer": {"ticker": issuer.ticker, "cik": issuer.cik, "issuer_name": issuer.issuer_name}, "history_backed": result, "controlled_outcome": case}
    ...
    report = {"schema_version": "FINSIGHT-BATCH-43-HISTORY-REPORT-1", "batch": 43, "valuation_date": BATCH_43_VALUATION_DATE, "policy_version": BATCH_43_HISTORY_VERSION, "denominator_tickers": list(BATCH_43_TICKERS), "attempted_count": 10, "pass_count": sum(row["outcome"] == "pass" for row in cases), "conditional_count": sum(row["outcome"] == "conditional" for row in cases), "withheld_count": sum(row["outcome"] == "withheld" for row in cases), "numeric_count": sum(row["outcome"] != "withheld" for row in cases), "pass_tickers": [row["ticker"] for row in cases if row["outcome"] == "pass"], "conditional_tickers": [row["ticker"] for row in cases if row["outcome"] == "conditional"], "withheld_tickers": [row["ticker"] for row in cases if row["outcome"] == "withheld"], "reliability_counts": reliability_counts, "serving_artifacts_changed": False, "serving_hash_before": before, "serving_hash_after": after, "watchlist_changed": False, "watchlist_sha256": watchlist_hash, "withheld_register_changed": False, "withheld_register_sha256": withheld_hash, "batch_42_dependency_status": "confirmed_batch_42_recovery_catalog_and_bookkeeping_bound", "cases": cases}
    _immutable(output_root / "batch-43-report.json", _json(report))
    return report
...
def main():
    ...
    print(json.dumps({key: report[key] for key in ("attempted_count", "pass_count", "conditional_count", "withheld_count", "numeric_count", "reliability_counts", "batch_42_dependency_status")}, sort_keys=True))

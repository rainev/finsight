...
## Pre-flight
1. Read `<project>/.codex/goodbehavior/manifest.json`. Missing → the project hasn't adopted GoodBehavior; tell the user
...
`updatedAt`) and prints a JSON report; exit 2 means conflicts need the human. It never touches `hooks.json`.
...
**`hooks.json` is special — never overwrite it.** It's project-owned and may carry the user's own hooks. Only update
`done-gate.py` itself. If the upstream `Stop`-hook wiring format changed, surface that as a note for the user to apply by
hand rather than rewriting their `hooks.json`.

**`AGENTS.md` principles** are tracked only if `$adopt-goodbehavior` recorded them in the manifest. If not (they were appended as prose
into a project-owned AGENTS.md), don't touch AGENTS.md — just report that upstream principles changed and let the user
...
> Requires `git` and a reachable, committed source repo. Touches only tracked GoodBehavior files under the project's
> `.agents/` and `.codex/` (and never `hooks.json`'s contents beyond the done-gate wiring). When unsure whether a
> divergence is intentional, prefer a conflict marker over a silent overwrite — same human-in-the-loop principle as the done-gate.

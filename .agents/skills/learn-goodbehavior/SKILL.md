...
Write a durable learning to project memory — `.codex/goodbehavior/memory/` (index `MEMORY.md`), unless the project
...
- **General discipline** — portable rule that would improve GoodBehavior everywhere. Record the local memory first, then
  propose the identity-stripped generic rewrite into `AGENTS.md` and/or the relevant skill. Ask before editing
...
```
Then add a one-line pointer to the memory index (`.codex/goodbehavior/memory/MEMORY.md`): `- [Title](file.md) — hook`.

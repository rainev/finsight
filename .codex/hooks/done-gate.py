...
Wire as a Stop hook in .codex/hooks.json. Requires only python3.
"""
import sys, json, re

# Tools whose use means the turn did real build work (lowercased) — arms the gate.
# Codex names (apply_patch/exec_command/exec/write_stdin) sit beside the generic ones.
BUILD_TOOLS = {"edit", "write", "multiedit", "notebookedit", "applypatch", "bash",
               "apply_patch", "exec_command", "exec", "write_stdin"}

# Tools that MUTATE files (for the behavioral check: what was the last change?).
MUTATION_TOOLS = {"edit", "write", "multiedit", "notebookedit", "applypatch", "apply_patch"}
...
OBSERVE_TOOLS = {"bash", "webfetch",
                 "exec_command", "exec", "write_stdin", "web_search_call", "view_image",
                 "view_image_tool_call", "navigate_page", "new_page", "take_snapshot", "click",
                 "fill_form", "press_key", "wait_for", "list_console_messages",
                 "list_network_requests", "list_pages"}
...
def main():
    ...
    path = data.get("transcript_path") or data.get("agent_transcript_path", "")
    entries = load_entries(path)

    text = data.get("last_assistant_message") or last_assistant_text(entries)
    if not text:
        sys.exit(0)
    t = text.lower()
...
def mutation_paths(name, tool_input):
    """Files a mutation touched. Codex apply_patch carries them in the patch header."""
...
def doc_only_mutations(tools):
    ...
    for name, tool_input in tools:
        if name not in MUTATION_TOOLS:
            continue
        for p in mutation_paths(name, tool_input):
            paths.append(str(p).strip().lower())
    if not paths:
        return False
...
def codex_payload(e):
    """The payload of a Codex rollout line (response_item/event_msg), else None."""
    if isinstance(e, dict) and e.get("type") in ("response_item", "event_msg"):
        p = e.get("payload")
...
def is_human_turn(e):
    """A real human message — not a tool_result / injected developer or tool text."""
    p = codex_payload(e)
    if p is not None:  # Codex rollout schema
        if p.get("type") == "item_completed" and isinstance(p.get("item"), dict):
            return p["item"].get("type") == "UserMessage"
        if p.get("type") == "message" and p.get("role") == "user":
            return any(isinstance(b, dict) and b.get("text", "").strip() for b in p.get("content", []))
        return False
...
def tool_uses_in(e):
    """The (name, input) tool calls in one entry — both Codex and generic schemas."""
    p = codex_payload(e)
    if p is not None:
        pt = p.get("type", "")
        if pt == "function_call":
            ...
            try:
                ...
                args = {}
            return [((p.get("name") or "").lower(), args)]
        if pt == "custom_tool_call":
            return [((p.get("name") or "").lower(), {"input": p.get("input", "")})]
        if pt in ("web_search_call", "tool_search_call", "view_image_tool_call"):
            return [(pt, {})]
        return []
...
def turn_tool_uses(entries):
    """All (tool_name, tool_input) used since the last human message, in order."""
    last_human = -1
    ...
    for e in entries[last_human + 1:]:
        out.extend(tool_uses_in(e))
    return out


def codex_message_text(p):
    """Text of a Codex assistant message payload (output_text/text blocks)."""
    if p.get("type") == "message" and p.get("role") == "assistant":
        return " ".join(b.get("text", "") for b in p.get("content", [])
                        if isinstance(b, dict) and b.get("type") in ("output_text", "text"))
    if p.get("type") == "task_complete":
        return p.get("last_agent_message", "") or ""
    return ""
